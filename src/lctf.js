...
_lctf.parse = function (data, offset, length, font, subt) {
  var bin = _bin;
  var obj = {};
  var offset0 = offset;
  ...
  obj.scriptList = _lctf.readScriptList(data, offset0 + offScriptList);
  obj.featureList = _lctf.readFeatureList(data, offset0 + offFeatureList);
  obj.lookupList = _lctf.readLookupList(data, offset0 + offLookupList, subt);

  return obj;
}

_lctf.readLookupList = function (data, offset, subt) {
  var bin = _bin;
  var offset0 = offset;
  var obj = [];
  ...
  for (var i = 0; i < count; i++) {
    ...
    offset += 2;
    var lut = _lctf.readLookupTable(data, offset0 + noff, subt);
    obj.push(lut);
  }
  return obj;
}

_lctf.readLookupTable = function (data, offset, subt) {
...
_lctf.numOfOnes = function (n) {
  var num = 0;
  for (var i = 0; i < 32; i++) if (((n >>> i) & 1) != 0) num++;
  return num;
}

_lctf.readClassDef = function (data, offset) {
  var bin = _bin;
  ...
  if (format == 1) {
    ...
    for (var i = 0; i < glyphCount; i++) {
      ...
      obj.class.push(bin.readUshort(data, offset));
      offset += 2;
    }
  }
  if (format == 2) {
    ...
    offset += 2;
    for (var i = 0; i < count; i++) {
      obj.start.push(bin.readUshort(data, offset));
      offset += 2;
      obj.end.push(bin.readUshort(data, offset));
      offset += 2;
      obj.class.push(bin.readUshort(data, offset));
      offset += 2;
    }
...
_lctf.readValueRecord = function (data, offset, valFmt) {
  var bin = _bin;
  var arr = [];
...
_lctf.readCoverage = function (data, offset) {
  var bin = _bin;
  var cvg = {};
...
_lctf.coverageIndex = function (cvg, val) {
  var tab = cvg.tab;
...
_lctf.readFeatureList = function (data, offset) {
  var bin = _bin;
  var offset0 = offset;
  var obj = [];
  ...
  for (var i = 0; i < count; i++) {
    ...
    var noff = bin.readUshort(data, offset);
    offset += 2;
    obj.push({tag: tag.trim(), tab: _lctf.readFeatureTable(data, offset0 + noff)});
  }
  return obj;
}

_lctf.readFeatureTable = function (data, offset) {
  var bin = _bin;
...
_lctf.readScriptList = function (data, offset) {
  var bin = _bin;
  var offset0 = offset;
  var obj = {};
  ...
  for (var i = 0; i < count; i++) {
    ...
    var noff = bin.readUshort(data, offset);
    offset += 2;
    obj[tag.trim()] = _lctf.readScriptTable(data, offset0 + noff);
  }
  return obj;
}

_lctf.readScriptTable = function (data, offset) {
  var bin = _bin;
  var offset0 = offset;
  var obj = {};
  ...
  offset += 2;
  obj.default = _lctf.readLangSysTable(data, offset0 + defLangSysOff);
  ...
  for (var i = 0; i < langSysCount; i++) {
    ...
    offset += 2;
    obj[tag.trim()] = _lctf.readLangSysTable(data, offset0 + langSysOff);
  }
  return obj;
}

_lctf.readLangSysTable = function (data, offset) {
  var bin = _bin;
  var obj = {};
